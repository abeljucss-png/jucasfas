(function () {
  ...
  if (Object.keys(campaign).length) {
    try { sessionStorage.setItem('opiniao_real_campaign', JSON.stringify(campaign)); } catch (_) {}
  } else {
    try { campaign = JSON.parse(sessionStorage.getItem('opiniao_real_campaign') || '{}'); } catch (_) { campaign = {}; }
  }
  ...
  function loadGtag(id) {
    ...
    s.dataset.opiniaoGa4 = 'true';
    document.head.appendChild(s);
    window.gtag = window.gtag || function () { dataLayer.push(arguments); };
    window.gtag('js', new Date());
    window.gtag('config', id, { send_page_view: false, allow_google_signals: true });
  }
  ...
  function loadGtm(id) {
    ...
    s.dataset.opiniaoGtm = 'true';
    document.head.appendChild(s);
    dataLayer.push({ 'gtm.start': new Date().getTime(), event: 'gtm.js' });
    push('gtm_ready', { gtm_container_id: id });
  }

  loadGtm(config.gtmContainerId);
  loadGtag(config.ga4MeasurementId);
  ...
  function scrollDepth() {
    ...
    if (percent <= lastScroll) return;
    lastScroll = percent;
    milestones.forEach(function (mark) {
      if (percent >= mark && !sent[mark]) {
        sent[mark] = true;
        push('scroll_depth', Object.assign({ depth_percent: mark }, pageContext()));
      }
    });
  }
  ...
  function closestProduct(el) {
    ...
    return {
      ...
      product_name: root && root.dataset.productName || title && title.textContent.trim() || ''
    };
  }

  document.addEventListener('click', function (event) {
    var el = event.target.closest('a,button,summary');
    if (!el) return;

    if (el.matches('[data-affiliate], a[href*="mercadolivre.com"], a[href*="amazon."]')) {
      var product = closestProduct(el);
      var card = el.closest('[data-cta-position], .productCard, .detailProduct, .comparisonSection, .hero, .buyingGuide');
      var position = el.dataset.ctaPosition || card && card.dataset.ctaPosition || card && card.className.split(' ')[0] || 'unknown';
      push('affiliate_click', Object.assign({
        category: el.dataset.category || document.body.dataset.category || 'unknown',
    ...
    if (el.matches('summary, [data-faq], .faqItem button')) {
      var faq = el.closest('details, [data-faq], .faqItem');
      var question = faq && (faq.querySelector('summary') || faq.querySelector('button'));
  ...
  var observer = 'IntersectionObserver' in window ? new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      ...
      if (el.matches('[data-product-view], .detailProduct, .productCard')) {
        push('product_view', Object.assign({ page: window.location.pathname }, closestProduct(el)));
      }
      if (el.matches('[data-comparison-view], .comparisonSection')) {
        push('comparison_view', { page: window.location.pathname, section: el.dataset.section || 'comparison' });
      }
    });
  }, { threshold: 0.35 }) : null;
  ...
  function experiment() {
    var requested = (params.get('ab_variant') || '').toLowerCase();
    var stored = '';
    ...
    document.querySelectorAll('[data-ab-test]').forEach(function (el) {
      var allowed = el.dataset.abVariant;
      if (allowed) el.hidden = allowed !== variant;
    });
    document.querySelectorAll('[data-ab-test]').forEach(function (el) {
      if (el.dataset.abVariant === variant && !el.dataset.abTracked) {
        el.dataset.abTracked = 'true';
        push('experiment_exposure', { experiment: el.dataset.abTest, variant: variant, page: window.location.pathname });
      }
    });
  }
  experiment();
  ...
  function routeChanged() {
    setTimeout(function () { push('page_view', pageContext()); }, 0);
  }
