...
defunction_exists('wp_body_open') || function wp_body_open() { do_action('wp_body_open'); };
...
function or_register_content_types() {
    register_post_type('seo_article',array(
        'labels'=>array('name'=>'Artigos SEO','singular_name'=>'Artigo SEO'),
        'public'=>true,'show_in_rest'=>true,'has_archive'=>true,'rewrite'=>array('slug'=>'artigos'),
        'supports'=>array('title','editor','excerpt','thumbnail','revisions','author'),
        'menu_icon'=>'dashicons-edit-page',
    ));
    register_post_type('product',array(
        'labels'=>array('name'=>'Produtos','singular_name'=>'Produto'),
        'public'=>true,'show_in_rest'=>true,'has_archive'=>true,'rewrite'=>array('slug'=>'produtos'),
        'supports'=>array('title','editor','excerpt','thumbnail','revisions'),
        'menu_icon'=>'dashicons-products',
    ));
    register_post_type('comparison',array(
        'labels'=>array('name'=>'Comparativos','singular_name'=>'Comparativo'),
        'public'=>true,'show_in_rest'=>true,'has_archive'=>true,'rewrite'=>array('slug'=>'comparativos'),
        'supports'=>array('title','editor','excerpt','thumbnail','revisions'),
        'menu_icon'=>'dashicons-chart-bar',
    ));
    register_taxonomy('topic',array('seo_article','product','comparison'),array(
        'labels'=>array('name'=>'Tópicos','singular_name'=>'Tópico'),'public'=>true,'show_in_rest'=>true,'hierarchical'=>true,'rewrite'=>array('slug'=>'categoria'),
    ));
    register_taxonomy('content_type',array('seo_article'),array(
        'labels'=>array('name'=>'Tipo editorial','singular_name'=>'Tipo editorial'),'public'=>true,'show_in_rest'=>true,'hierarchical'=>false,'rewrite'=>array('slug'=>'tipo'),
    ));
}
add_action('init','or_register_content_types');

function or_elementor_compatibility() {
    if (did_action('elementor/loaded')) {
        add_post_type_support('page','elementor');
        add_post_type_support('seo_article','elementor');
        add_post_type_support('comparison','elementor');
    }
}
add_action('init','or_elementor_compatibility',20);

function or_rank_math_breadcrumbs() {
...
function or_fallback_breadcrumbs() {
    ...
    if (is_singular()) {
        ...
        if ($obj instanceof WP_Post) {
            $topics=get_the_terms($obj->ID,'topic');
            if ($topics && !is_wp_error($topics)) echo '<a href="'.esc_url(get_term_link($topics[0])).'">'.esc_html($topics[0]->name).'</a><span aria-hidden="true"> / </span>';
...
function or_affiliate_url($url) {
    return esc_url($url);
}

function or_schema_fallback() {
    if (class_exists('RankMath\\Schema\\JsonLD\\JsonLD')) return;
    if (!is_singular(array('post','seo_article','comparison','product'))) return;
